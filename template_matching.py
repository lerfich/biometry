...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        photoButton.clicked.connect(
            lambda: self.upload_photo(photoInput.text()))

        self.globalLayout.addWidget(photo, 1, 0)
        self.globalLayout.addWidget(photoInput, 1, 1)
        self.globalLayout.addWidget(photoButton, 1, 3)
    ...
    def upload_photo(self, path):
        ...
        vLayout = QVBoxLayout()
        findButton = QPushButton("Use template matching to search")
        findButton.clicked.connect(lambda: self.findFace(path))
        findButton2 = QPushButton("Use Viola-Jones")
        findButton2.clicked.connect(lambda: self.viola(path))
        vLayout.addWidget(findButton)
        vLayout.addWidget(findButton2)
        widget = QWidget()
        widget.setLayout(vLayout)
        self.globalLayout.addWidget(lbl, 1, 10)
        self.globalLayout.addWidget(widget, 1, 1)

    def findFace(self, path) -> None:
        template = cv2.imread('zo_template.jpg', 0)
        img = cv2.imread(path)
